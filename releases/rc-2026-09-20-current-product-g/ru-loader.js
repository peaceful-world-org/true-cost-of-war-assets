(()=>{
  ...
  const overviewSrc='https://peaceful-world-org.github.io/true-cost-of-war/unified/product.html?lang=ru&embed=1';
  const detailsSrc='https://peaceful-world-org.github.io/true-cost-of-war/model-v2-lab/index.html?lang=ru&embed=1';
  ...
  chromeStyle.textContent=`
    ...
    #pw-tcow-parent-session-counter{
      ...
      display:flex;min-width:214px;max-width:232px;flex-direction:column;gap:4px;
      padding:11px 14px 10px;border:1px solid rgba(201,196,184,.86);border-radius:15px;
    ...
    #pw-tcow-parent-session-counter .pw-live-label{
      ...
      font:780 9px/1.2 Inter,system-ui,sans-serif;letter-spacing:.05em;text-transform:uppercase;
      white-space:nowrap;
    }
    ...
    #pw-tcow-parent-session-counter .pw-live-value{
      color:#181a17;font:850 21px/1.12 Inter,system-ui,sans-serif;
      font-variant-numeric:tabular-nums;letter-spacing:-.025em;white-space:nowrap;
    }
    #pw-tcow-parent-session-counter .pw-live-note{
      margin-top:1px;color:#696b65;font:570 9.8px/1.3 Inter,system-ui,sans-serif;
      letter-spacing:.005em;white-space:normal;
    }
    @media(max-width:980px){
      #pw-tcow-parent-session-counter{
        top:auto;right:auto;left:50%;
        bottom:calc(62px + env(safe-area-inset-bottom,0px));
        min-width:210px;max-width:calc(100vw - 24px);padding:9px 12px 8px;border-radius:13px;
        transform:translate(-50%,8px) scale(.98);
      }
      #pw-tcow-parent-session-counter.is-visible{transform:translate(-50%,0) scale(1)}
      #pw-tcow-parent-session-counter .pw-live-label{font-size:8.8px}
      #pw-tcow-parent-session-counter .pw-live-value{font-size:18px}
      #pw-tcow-parent-session-counter .pw-live-note{font-size:9.2px}
    }
  ...
  sessionCounter.setAttribute('aria-label',lang==='ru'?'Мировые военные расходы за время просмотра':'World military spending during your viewing session');
  ...
  sessionCounter.innerHTML=`
    <div class="pw-live-label"><span class="pw-live-dot" aria-hidden="true"></span><span>${lang==='ru'?'ЗА ВРЕМЯ ПРОСМОТРА':'DURING YOUR VIEW'}</span></div>
    <div class="pw-live-value" data-live-value>≈ $ 0</div>
    <div class="pw-live-note">${lang==='ru'?'мировые военные расходы · средний темп 2025':'world military spending · 2025 average rate'}</div>
  `;
